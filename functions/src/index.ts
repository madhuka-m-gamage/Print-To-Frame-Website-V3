...
export const submitLead = onCall({ secrets: [GMAIL_APP_PASSWORD] }, async (request) => {
  ...
  try {
    lead = validatePayload(request.data);
  } catch (err) {
    console.error('Lead submission rejected: invalid payload', err, request.data);
    throw err;
  }

  const { name, phone, intent } = lead;
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px;">
          ...
          <p><strong>Phone:</strong> +94 ${phone}</p>
          <p><strong>Requirement:</strong> ${intent}</p>
